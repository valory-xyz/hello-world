...
AGENT_NAME := valory/hello_world
IMAGE_NAME := valory/oar-hello_world
release-image:
	$(eval AGENT_HASH := $(shell cat ${PACKAGES_PATH} | grep "agent/${AGENT_NAME}" | cut -d "\"" -f4 ))
	$(eval AGENT_PUBLIC_ID := ${AGENT_NAME}:${RELEASE_VERSION}:${AGENT_HASH})
	...
	autonomy push-all
	autonomy build-image ${AGENT_PUBLIC_ID} --pull
	docker push ${IMAGE_NAME}:${AGENT_HASH}
